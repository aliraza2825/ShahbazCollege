				...
				<div class="col-md-12 ">
					...
					<div class="portlet box green ">
						...
						<div class="portlet-body form">
                            <form class="form-horizontal" role="form" method="post" action="<?php echo site_url();?>/assignments/check_assignments">
								<div class="form-body">
						...
						<div class="portlet-body">
							...
							<?php
								...
								foreach($assignments as $assignment):
                                    //RESULT DETAILS
                                    $result_details = $this->db->get_where('assignment_results',array('student_id'=>$assignment['student_id'],'assignment_id'=>$assignment['assignment_id']))->result_array();
                                    $marks=0;
                                    if(count($result_details)>0)
                                    {
                                        $mcqs_marks = json_decode($result_details[0]['obtain_mcqs_marks']);
                                        $short_questions_marks = json_decode($result_details[0]['obtain_short_questions_marks']);
                                        $practicals_marks = json_decode($result_details[0]['obtain_practical_marks']);

                                        foreach($mcqs_marks as $mcq_marks)
                                        {
                                            $marks+=$mcq_marks;
                                        }
                                        foreach($short_questions_marks as $short_question_marks)
                                        {
                                            $marks+=$short_question_marks;
                                        }
                                        foreach($practicals_marks as $practical_marks)
                                        {
                            ...
                            <tr class="odd gradeX">
                                ...
                                <td>
                                    <?php echo @$marks.'/'.$assignment['total_marks'];?>
                                </td>
                                <td>
                                    <?php echo $result_details[0]['check_by'];?>
                                </td>
                                <td>
