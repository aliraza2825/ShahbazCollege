...
class Posapi extends CI_Controller {
	...
	private function _header_meta($user)
	{
		$is_admin = $this->_is_admin($user);
		$access = $this->_pos_access_row($user);
		$flag = function ($key) use ($is_admin, $access) {
			return $this->_header_flag($is_admin, $access, $key);
		};
		...
		$accounts = array(
			'account_details' => $flag('account_details'),
			'profit_distribution' => $flag('profit_distribution'),
			'chart_of_accounts' => $flag('chart_of_accounts'),
			'campus_petty_cash' => $flag('campus_petty_cash'),
			'advance_system' => $flag('advance_system'),
			'loan_approval_accounts' => $flag('loan_approval_accounts'),
			'dailyclosing' => $flag('dailyclosing'),
			'closing_reconcile' => $flag('closing_reconcile'),
			'bank_reconciliation' => $flag('bank_reconciliation'),
			'paypro_settlements' => $flag('bank_reconciliation'),
			'paypro_untagged' => $flag('bank_reconciliation'),
			'day_closing_report' => $flag('bank_reconciliation'),
		);
		$students = array(
			'student_add' => $flag('student_add'),
			'student_all' => $flag('student_all'),
		);
		$visitors = array(
			'visitor_add' => $flag('visitor_add'),
			'visitor_all' => $flag('visitor_all'),
		);

		return array(
			'designations' => $designations,
			'petty_cash' => $petty_cash,
			'show_accounts_menu' => $is_admin || in_array(true, $accounts, true),
			'accounts' => $accounts,
			'show_students_menu' => $is_admin || in_array(true, $students, true),
			'students' => $students,
			'show_expenses_menu' => $is_admin || ($access && !empty($access['expense_sidebar'])),
			'expense_add' => $flag('expense_add'),
			'expense_all' => $flag('expense_all'),
			'show_visitors_menu' => $is_admin || in_array(true, $visitors, true),
			'visitors' => $visitors,
