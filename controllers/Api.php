...
// 	}
		...
		foreach($lectures as $lecture)
		{
			...
			foreach($subjects as $subject)
			{
				...
				foreach($lecture_details as $lecture_detail)
				{
					...
					{
						$lecture_subjects[$a]['date_time'][$b]['topics'][] = array(
					...
					{
						$lecture_subjects[$a]['date_time'][$b]['practicals'][] = array(
			...
			foreach($subjects as $subject)
			{
				...
				foreach($lecture_details as $lecture_detail)
				{
					...
					foreach($topics as $topic)
					{
						...
						$studied = $this->db->get_where('study_by_teacher', array(
							'topic_id' => $topic['topic_id'],
							'session_syllabus_id' => $lecture_detail['id'],
							'is_quiz' => $lecture_detail['is_quiz'],
						))->row_array();
						if (!empty($studied)) {
							$topic_row['is_studied'] = true;
							$topic_row['studied_by'] = $studied['created_by'];
							$topic_row['studied_at'] = $studied['created_at'];
						}
						$lecture_subjects[$a]['date_time'][$b]['topics'][] = $topic_row;
					}
					...
					foreach($practicals as $practical)
					{
						...
						if (!empty($studied)) {
							$practical_row['is_studied'] = true;
							$practical_row['studied_by'] = $studied['created_by'];
							$practical_row['studied_at'] = $studied['created_at'];
						}
						$lecture_subjects[$a]['date_time'][$b]['practicals'][] = $practical_row;
					}
    ...
    public function getZoomMeetings()
    {
        ...
        foreach ($rows as $row) {
            $meetingId = '';
            if (isset($row['personal_meeting_id'])) {
            ...
            $response[] = array(
                'zoom_id' => isset($row['zoom_id']) ? (string) $row['zoom_id'] : '',
                'personal_meeting_id' => $meetingId,
                'admission_timing' => isset($row['admission_timing']) ? (string) $row['admission_timing'] : '',
