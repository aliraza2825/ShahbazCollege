...
/**
 * Course Sessions JSON API for React POS shell.
 * Base: /index.php/coursesessionsapi/{method}
 */
class Coursesessionsapi extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->library('course_session_service');
		$this->service = $this->course_session_service;
	...
	{
		$this->_json(array('success' => true, 'data' => $this->service->list_all()));
	}
	...
	public function session($id = 0)
	{
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			$result = $this->service->update($id, $this->_body());
	...
	{
		$result = $this->service->create($this->_body());
		if (empty($result['success'])) {
			$this->_json($result, 422);
		}
		$this->_json($result);
	...
	public function delete_session($id = 0)
	{
	...
	public function exams()
	{
		...
		$this->_json(array('success' => true, 'data' => $this->service->exams_for_course($course_id)));
	}

	public function council_preview()
	{
		$course_id = (int) $this->input->get('course_id');
		$exam = $this->input->get('exam');
		$this->_json(array(
			'success' => true,
			'data' => $this->service->council_sequence_preview($course_id, $exam),
		));
	}
}
