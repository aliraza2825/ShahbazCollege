...
class Construction extends CI_Controller {
    ...
    private function redirect_after_save($default)
    {
        $redirectTo = $this->input->post('redirect_to');
        if ($redirectTo && strpos($redirectTo, 'construction/') === 0) {
            redirect($redirectTo);
            return;
        }
        redirect($default);
    ...
    private function contractor_total_cost($projectId = null)
    {
        $workAmount = $this->contractor_work_amount($projectId);
        $paidAmount = $this->contractor_paid_amount($projectId);
        return max($workAmount, $paidAmount);
    }
    ...
    private function contractor_work_amount($projectId = null)
    {
        $this->db->select('SUM(CASE WHEN final_bill > 0 THEN final_bill ELSE contract_amount END) as total', false);
        if ($projectId !== null) {
            $this->db->where('project_id', $projectId);
        }
        $row = $this->db->get('construction_contractors')->row_array();
        return (float) @$row['total'];
    }

    private function contractor_paid_amount($projectId = null)
    {
        $this->db->select_sum('amount');
        if ($projectId !== null) {
            $this->db->where('project_id', $projectId);
        }
        $row = $this->db->get('construction_contractor_payments')->row_array();
        return (float) @$row['amount'];
    }
    ...
    private function recent_cost_entries($limit = 10)
    {
        ...
        $contractor = $this->db
            ->select("DATE(construction_contractors.created_at) as date, 'Contractor Work' as type, CASE WHEN construction_contractors.final_bill > 0 THEN construction_contractors.final_bill ELSE construction_contractors.contract_amount END as amount, construction_contractors.contact_details as description, construction_projects.project_name, construction_contractors.contractor_name as product_name, 0 as quantity", false)
        ...
        $rows = array_merge($generic, $material, $contractor);
    ...
    private function contractor_summary($projectId = null)
    {
        $this->db
            ...
            ->order_by('construction_contractors.id', 'DESC');
        if ($projectId !== null) {
            $this->db->where('construction_contractors.project_id', $projectId);
        }
        $contractors = $this->db->get('construction_contractors')->result_array();
    ...
    public function index()
    {
        ...
        $data['contractor_cost'] = $this->contractor_total_cost() + $this->sum_simple_entries('Contractor');
        $data['contractor_paid'] = $this->contractor_paid_amount();
        $data['contractor_remaining'] = $data['contractor_cost'] - $data['contractor_paid'];
    ...
    public function project($projectId)
    {
        ...
        $project = $this->db->where('id', $projectId)->get('construction_projects')->row_array();
        if (!$project) {
        ...
        $costRows = $this->project_cost_rows($projectId);
        $data['project'] = $project;
        $data['project_cost'] = !empty($costRows) ? $costRows[0] : $project;
        $data['users'] = $this->db->where('status', 1)->order_by('first_name', 'ASC')->get('users')->result_array();
        ...
        $data['boq'] = $this->db
            ...
            ->result_array();
        $data['issues'] = $this->db
            ->select('construction_material_issues.*, product_names.product_name')
            ->join('product_names', 'product_names.product_name_id = construction_material_issues.product_name_id', 'left')
            ->where('construction_material_issues.project_id', $projectId)
            ->order_by('construction_material_issues.id', 'DESC')
            ->get('construction_material_issues')
            ->result_array();
        $data['contractors'] = $this->contractor_summary($projectId);
        $data['contractor_payments'] = $this->db
            ->select('construction_contractor_payments.*, construction_contractors.contractor_name')
            ->join('construction_contractors', 'construction_contractors.id = construction_contractor_payments.contractor_id', 'left')
            ->where('construction_contractor_payments.project_id', $projectId)
        ...
        $data['progress'] = $this->db
            ->where('project_id', $projectId)
            ->order_by('progress_date', 'DESC')
            ->get('construction_progress')
            ->result_array();

        $this->page('project_detail', $data);
    }
    ...
    public function save_project()
    {
        ...
        $this->redirect_after_save('construction/projects');
    }
    ...
    public function save_boq()
    {
        ...
        $this->redirect_after_save('construction/boq');
    }
    ...
    public function save_material_issue()
    {
        ...
        $this->redirect_after_save('construction/work');
    }
    ...
    public function save_entry()
    {
        ...
        if ($type === 'Material') {
            ...
            $this->session->set_flashdata($result['success'] ? 'message' : 'error', $result['message']);
            $this->redirect_after_save('construction/entries');
            return;
        }
    ...
    public function save_contractor()
    {
        ...
        $this->redirect_after_save('construction/contractors');
    }
    ...
    private function project_cost_rows($onlyProjectId = null)
    {
        if ($onlyProjectId !== null) {
            $this->db->where('id', $onlyProjectId);
        }
        ...
        foreach ($projects as $key => $project) {
            ...
            $contractor = $this->contractor_total_cost($projectId) + $this->sum_simple_entries('Contractor', $projectId);
            $contractorPaid = $this->contractor_paid_amount($projectId);
            ...
            $projects[$key]['contractor_paid'] = $contractorPaid;
            $projects[$key]['contractor_remaining'] = $contractor - $contractorPaid;
