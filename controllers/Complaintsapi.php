...
/**
 * Complaints JSON API for React POS shell.
 * Base: /index.php/complaintsapi/{method}
 */
class Complaintsapi extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->library('complaints_service');
		$this->service = $this->complaints_service;
		if (!$this->service->can_manage($this->current_user)) {
			$this->_json(array('success' => false, 'message' => 'Complaints access required'), 403);
		}
	}
	...
	{
		$this->_json(array('success' => true, 'data' => $this->service->list_by_status(0)));
	}

	public function completed()
	{
		$this->_json(array('success' => true, 'data' => $this->service->list_by_status(1)));
	}

	public function detail($complaint_id = null)
	{
		$row = $this->service->get_detail($complaint_id);
		if (!$row) {
	...
	{
		$body = $this->_body();
		$result = $this->service->reply($complaint_id, isset($body['message']) ? $body['message'] : '', $this->current_user['user_id']);
	...
	public function solved($complaint_id = null)
	{
		$this->_json($this->service->mark_solved($complaint_id));
	}
}
