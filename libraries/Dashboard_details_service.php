...
class Dashboard_details_service {
    ...
    private function _batch_student_photo_docs($student_ids)
    {
        if (!count($student_ids)) {
            return array();
        }
        $this->ci->db->reset_query();
        $this->ci->db->select('student_id, image, online_image');
        $this->ci->db->from('student_documents');
        $this->ci->db->where_in('student_id', $student_ids);
        $this->ci->db->where('type', 'Photo');
        $rows = $this->ci->db->get()->result_array();
        $map = array();
        foreach ($rows as $d) {
            $sid = (int) $d['student_id'];
            ...
            $photo_url = '';
            if (!empty($d['online_image'])) {
                $photo_url = $d['online_image'];
            } elseif (!empty($d['image'])) {
                $photo_url = rtrim(base_url(), '/').'/uploads/'.$d['image'];
            }
            $map[$sid] = $photo_url;
        }
        return $map;
    ...
    /** Legacy new_admissions.php payment block. */
    private function _build_legacy_payment_details($payments, $student_total_fee)
    {
        $total_fee = 0;
        ...
        $paid_details = array();
        $unpaid_details = array();
        $today = date('Y-m-d');

        foreach ($payments as $payment) {
            $plan = isset($payment['payment_plan']) ? $payment['payment_plan'] : '';
            $amount = (float) (isset($payment['amount']) ? $payment['amount'] : 0);
            if ($plan !== 'consulation fee') {
                $total_fee += $amount;
            }
            if ($plan === 'consulation fee') {
                $created_council_fee += $amount;
                if (!empty($payment['paid'])) {
                    $submitted_council_fee += (float) (isset($payment['actual_amount']) ? $payment['actual_amount'] : 0);
                }
            }
            $deadline = isset($payment['dead_line']) ? $payment['dead_line'] : '';
            if ($deadline !== '' && $deadline < $today) {
                $fee_decided_current_time += $amount;
                if (empty($payment['paid'])) {
            ...
            if (!empty($payment['paid'])) {
                $paid_details[] = array(
                    'text' => (isset($payment['actual_amount']) ? $payment['actual_amount'] : 0).' Paid on '.(isset($payment['paid_date']) ? $payment['paid_date'] : ''),
                    'overdue' => false,
                );
                if ($plan !== 'consulation fee') {
                    $total_fee_submitted += (float) (isset($payment['actual_amount']) ? $payment['actual_amount'] : 0);
                }
            } else {
                $overdue = ($deadline !== '' && $deadline < $today);
                $unpaid_details[] = array(
                    'text' => $amount.' Not Paid on '.$deadline,
                    'overdue' => $overdue,
                );
            }
        }
        ...
        $pct_received = $total_fee > 0 ? (int) round(($total_fee_submitted / $total_fee) * 100) : 0;
        $pct_decision = $fee_decided_current_time > 0 ? (int) round(($total_fee_submitted / $fee_decided_current_time) * 100) : 0;

        return array(
            'student_total_fee' => $student_total_fee,
            'total_created_fee' => $total_fee,
            'created_council_fee' => $created_council_fee,
            ...
            'percentage_paid_by_decision' => $pct_decision,
            'paid_details' => $paid_details,
            'unpaid_details' => $unpaid_details,
        );
    }

    private function _payments_for_student_report($payments_map, $student_id)
    {
        if (!isset($payments_map[$student_id])) {
            return array();
        }
        $all = array_merge($payments_map[$student_id]['paid'], $payments_map[$student_id]['unpaid']);
        usort($all, function ($a, $b) {
            $da = isset($a['dead_line']) ? $a['dead_line'] : '';
            $db = isset($b['dead_line']) ? $b['dead_line'] : '';
            if ($da === $db) {
                return 0;
            }
            return ($da < $db) ? -1 : 1;
    ...
    private function _fee_update_display_value($key, $value)
    {
        if ($key === 'paid') {
            return !empty($value) ? 'Paid' : 'Unpaid';
        }
        if ($key === 'college_fee') {
            return !empty($value) ? 'College Fee' : 'Bank Fee';
        }
        if ($key === 'scan_challan') {
    ...
    private function _fee_update_changes($request, $original)
    {
        $fields = array(
            array('key' => 'amount', 'label' => 'Fee amount'),
            array('key' => 'actual_amount', 'label' => 'Paid amount'),
            array('key' => 'remaining_installment_amount', 'label' => 'Previous installment'),
            array('key' => 'extra_amount', 'label' => 'Fine amount'),
            array('key' => 'dead_line', 'label' => 'Deadline'),
            array('key' => 'paid_date', 'label' => 'Paid date'),
            array('key' => 'paid', 'label' => 'Status'),
            array('key' => 'college_fee', 'label' => 'Fee type'),
            array('key' => 'scan_challan', 'label' => 'Challan scan'),
        );
        $changes = array();
        foreach ($fields as $field) {
            $key = $field['key'];
            $requested = isset($request[$key]) ? $request[$key] : '';
            $current = isset($original[$key]) ? $original[$key] : '';
            $req_display = $this->_fee_update_display_value($key, $requested);
            $cur_display = $this->_fee_update_display_value($key, $current);
            $changed = (string) $requested !== (string) $current;
            if ($changed) {
                $changes[] = array(
                    'field' => $field['label'],
                    'key' => $key,
                    'requested' => $req_display,
                    'current' => $cur_display,
                );
            }
        }
        if (!empty($request['del'])) {
            $changes[] = array(
                'field' => 'Action',
                'key' => 'del',
                'requested' => 'Delete payment',
                'current' => 'Keep payment',
            );
        }
        return $changes;
    }

    private function _fee_update_row($r)
    {
        ...
        $upload_base = rtrim(base_url(), '/').'/uploads/';
        $scan = isset($r['scan_challan']) ? $r['scan_challan'] : '';
        $orig_scan = isset($orig['scan_challan']) ? $orig['scan_challan'] : '';
        ...
        return array(
            ...
            'remaining_installment_amount' => isset($r['remaining_installment_amount']) ? $r['remaining_installment_amount'] : '',
            'extra_amount' => isset($r['extra_amount']) ? $r['extra_amount'] : '',
            'paid' => !empty($r['paid']),
            'dead_line' => isset($r['dead_line']) ? $r['dead_line'] : '',
            'paid_date' => isset($r['paid_date']) ? $r['paid_date'] : '',
            'college_fee' => !empty($r['college_fee']),
            'del' => !empty($r['del']),
            'party_label' => $party,
            'campus_name' => isset($r['campus_name']) ? $r['campus_name'] : '',
            'reason' => isset($r['reason']) ? $r['reason'] : '',
            'system_comment' => !empty($r['del']) ? 'Delete Payment' : (isset($r['system_comment']) ? $r['system_comment'] : ''),
            'scan_challan' => $scan,
            'scan_challan_url' => $scan !== '' ? $upload_base.$scan : '',
            'add_by' => isset($r['add_by']) ? $r['add_by'] : '',
            'last_edit' => isset($r['last_edit']) ? $r['last_edit'] : '',
            'requested_by' => isset($r['last_edit']) && $r['last_edit'] !== ''
                ? $r['last_edit']
                : (isset($r['add_by']) ? $r['add_by'] : ''),
            'changes' => $this->_fee_update_changes($r, $orig),
            'original' => array(
                'amount' => isset($orig['amount']) ? $orig['amount'] : '',
                'actual_amount' => isset($orig['actual_amount']) ? $orig['actual_amount'] : '',
                'remaining_installment_amount' => isset($orig['remaining_installment_amount']) ? $orig['remaining_installment_amount'] : '',
                'extra_amount' => isset($orig['extra_amount']) ? $orig['extra_amount'] : '',
                'paid' => !empty($orig['paid']),
                'dead_line' => isset($orig['dead_line']) ? $orig['dead_line'] : '',
                'paid_date' => isset($orig['paid_date']) ? $orig['paid_date'] : '',
                'college_fee' => !empty($orig['college_fee']),
                'scan_challan' => $orig_scan,
                'scan_challan_url' => $orig_scan !== '' ? $upload_base.$orig_scan : '',
            ),
        );
    }
    ...
    public function update_fee_requests($user, $campus_id = null, $kind = 'student')
    {
        ...
        if ($kind === 'contractor') {
            ...
            // Resolve campus scope BEFORE building the list query — get_where() resets CI QB.
            $campus_scope = $this->_campus_scope_for_user($user);
    ...
    public function approve_fee_update($user, $request_id)
    {
        ...
        // Fee approval must not change the student profile audit fields (legacy parity).
        $student_id = isset($update_request[0]['student_id']) ? (int) $update_request[0]['student_id'] : 0;
        $student_snapshot = null;
        if ($student_id > 0) {
            $student_snapshot = $this->ci->db->get_where('students', array('student_id' => $student_id))->row_array();
        }

        $this->ci->dashboards->updatePayment($update_request);
        $this->ci->db->set('ok_by_admin', '1');
        $this->ci->db->set('clear_by', $this->_name($user));
        $this->ci->db->where(array('id' => (int) $request_id, 'ok_by_admin' => 0));
        $this->ci->db->update('update_payment_requests');

        if ($student_snapshot) {
            $restore = array('last_edit' => $student_snapshot['last_edit']);
            if (array_key_exists('add_by', $student_snapshot)) {
                $restore['add_by'] = $student_snapshot['add_by'];
            }
            $this->ci->db->where('student_id', $student_id);
            $this->ci->db->update('students', $restore);
        }
    ...
    public function approve_discount($user, $discount_id)
    {
        ...
        $this->ci->db->set('approved_by', $this->ci->session->userdata('name'));
        ...
        $this->ci->db->where('student_id', $update_request[0]['student_id']);
        $this->ci->db->where(array('paid' => 0, 'payment_plan!=' => 'consulation fee'));
        ...
        return array('success' => true, 'message' => 'Discount Updated successfully.');
    }
    ...
    public function reject_discount($user, $discount_id)
    {
        ...
        $this->ci->db->update('discounts_approval');
        return array('success' => true, 'message' => 'Fee Request Rejected.');
    }
    ...
    public function new_admissions_report($user, $month = null, $year = null)
    {
        ...
        $payments_map = $this->_batch_payments_by_student($student_ids);
        $docs_map = $this->_batch_documents_by_student($student_ids);
        $photos_map = $this->_batch_student_photo_docs($student_ids);
        $contractors = $this->_batch_contractors($contractor_ids);

        $out = array();
        foreach ($rows as $r) {
            $sid = (int) $r['student_id'];
            $payments = $this->_payments_for_student_report($payments_map, $sid);
            $payment_meta = $this->_build_legacy_payment_details(
                $payments,
                isset($r['total_fee']) ? $r['total_fee'] : 0
            );

            $contractor = 'N/A';
            $contractor_id = !empty($r['contractor_id']) ? (int) $r['contractor_id'] : 0;
            if ($contractor_id && isset($contractors[$contractor_id])) {
                $c = $contractors[$contractor_id];
                $contractor = $c['name'].(isset($c['date']) ? ' ('.$c['date'].')' : '');
            }
            ...
            $out[] = array_merge(array(
                ...
                'roll_no' => isset($r['roll_no']) ? $r['roll_no'] : '',
                'cnic' => isset($r['cnic']) ? $r['cnic'] : '',
                'father_name' => isset($r['father_name']) ? $r['father_name'] : '',
                'mobile' => isset($r['mobile']) ? $r['mobile'] : '',
                'emergency_no' => isset($r['emergency_no']) ? $r['emergency_no'] : '',
                'campus_name' => isset($r['campus_name']) ? $r['campus_name'] : '',
                'class_name' => isset($r['class_name']) ? $r['class_name'] : '',
                ...
                'photo_url' => isset($photos_map[$sid]) ? $photos_map[$sid] : '',
                'documents' => isset($docs_map[$sid]) ? $docs_map[$sid] : array(),
            ), $payment_meta);
        }
