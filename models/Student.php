...
class Student extends CI_Model {
	...
	public function getCountDeletedFess($student_id)
	{
	    $query = $this->db->get_where('update_payment_requests',array('student_id'=>$student_id,'del'=>1,'ok_by_admin'=>1))->result_array();
	    return count($query);
	}
	
	public function getCountShiftedFess($student_id)
	{
	    $query = $this->db->get_where('update_payment_requests',array('student_id'=>$student_id,'del'=>0,'ok_by_admin'=>1))->result_array();
	    return count($query);
	}
	...
	public function challan($challan_id)
	{
		...
		$this->db->select('payments.*, students.first_name as first_name, students.last_name as last_name, students.roll_no as roll_no,students.cnic as cnic ,students.father_name as father_name, classes.name as class_name, campuses.bank_name, campuses.account_no, campuses.address, campuses.campus_name, campuses.note, campuses.campus_id');
		$this->db->from('payments');
	...
	{
		$all_created_fee_qry = "SELECT SUM(amount) as amount FROM `payments` WHERE student_id=$id AND payment_plan!='consulation fee' AND payment_comment='College Fee'";
		$all_created_fee = $this->db->query($all_created_fee_qry)->result_array();

        $all_reversal_fee_qry = "SELECT SUM(prr.reversal_amount) as reversal_amount FROM payments_reversal_requests as prr INNER JOIN payments as p ON p.id=prr.payment_id WHERE prr.student_id=$id AND prr.done=1 AND p.payment_comment='College Fee'";
        $all_reversal_fee = $this->db->query($all_reversal_fee_qry)->result_array();

        if(count($all_reversal_fee)>0)
        {
            $query = $all_created_fee[0]['amount']-$all_reversal_fee[0]['reversal_amount'];
        }
        else
        {
            $query = $all_created_fee[0]['amount'];
        }
    ...
    public function payment_paid_old($id,$payment_id)
    {
        $this->db->select('*');
        $this->db->from('archive_payments');
        $this->db->where('student_id', $id);
    ...
    public function getStudentDiscount_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as total_paid FROM `archive_payments` WHERE student_id=$id and payment_id = $payment_id AND payment_plan!='consulation fee' AND payment_comment='College Fee'";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }

    public function getStudentPaidFee_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as paid_fee FROM `archive_payments` WHERE student_id=$id and payment_id = $payment_id AND payment_plan!='consulation fee' AND payment_comment='College Fee' AND paid=1";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }

    public function getStudentTotalExtraFee_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as total_extra_fee FROM `archive_payments` WHERE student_id=$id and payment_id = $payment_id AND payment_plan!='consulation fee' AND payment_comment!='College Fee'";
        $query = $this->db->query($qry)->result_array();
        return $query[0]['total_extra_fee'];
    }

    public function getStudentTotalExtraPaidFee_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as total_paid_extra_fee FROM `archive_payments` WHERE student_id=$id and payment_id = $payment_id AND payment_plan!='consulation fee' AND payment_comment!='College Fee' AND paid=1";
        $query = $this->db->query($qry)->result_array();
        return $query[0]['total_paid_extra_fee'];
    }

    public function getStudentExtraFeePaidTillDate_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as extra_fee_paid_till_date FROM `archive_payments` WHERE student_id=$id and payment_id = $payment_id AND payment_plan!='consulation fee' AND payment_comment!='College Fee' AND paid=1 AND dead_line<= '".date('Y-m-d')."'";
        $query = $this->db->query($qry)->result_array();
        return $query[0]['extra_fee_paid_till_date'];
    }

    public function getStudentExtraFeeRemainingTillDate_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as extra_fee_remaining_till_date FROM `archive_payments` WHERE student_id=$id and payment_id = $payment_id AND payment_plan!='consulation fee' AND payment_comment!='College Fee' AND paid=0 AND dead_line<= '".date('Y-m-d')."'";
        $query = $this->db->query($qry)->result_array();
        return $query[0]['extra_fee_remaining_till_date'];
    }

    public function getStudentShiftDeleteFee_old($id,$payment_id)
    {
        $payments = $this->db->get_where('archive_payments',array('student_id'=>$id))->result_array();
        $payments_ids = array();
        foreach($payments as $payment)
        {
            array_push($payments_ids,$payment['id']);
    ...
    public function getStudentRemainingFee_old($id,$payment_id)
    {
        //$qry = "SELECT SUM(amount)-SUM(actual_amount) as remaining_fee FROM `payments` where student_id=$id AND payment_plan!='consulation fee'";
        //$query = $this->db->query($qry)->result_array();
        //return $query;
    ...
    public function getStudentFeeShouldPay_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as fee_should_pay FROM `archive_payments` where student_id=$id AND dead_line<= '".date('Y-m-d')."' and payment_id = $payment_id AND payment_plan!='consulation fee' AND payment_comment='College Fee'";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }

    public function getNextPaymentId_old($payment_id, $student_id)
    {
        $qry = "SELECT * FROM archive_payments where id!=$payment_id and paid=0 and payment_id = $payment_id and student_id=$student_id AND payment_plan!='consulation fee' ORDER BY dead_line ASC LIMIT 1";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }

    public function getNextPaymentIdContractor_old($payment_id, $contract_id)
    {
        $qry = "SELECT * FROM archive_payments where id>$payment_id and contract_id=$contract_id AND payment_plan!='consulation fee' LIMIT 1";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }

    public function getStudentConsulationFee_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as consulation_fee FROM `archive_payments` WHERE student_id=$id and payment_id = $payment_id AND payment_plan='consulation fee'";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }

    public function getStudentConsulationFeePaid_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as consulation_fee_paid FROM `archive_payments` WHERE student_id=$id AND payment_id = $payment_id AND payment_plan='consulation fee' AND paid=1";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }

    public function getStudentConsulationFeeUnPaid_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as consulation_fee_unpaid FROM `archive_payments` WHERE student_id=$id and payment_id = $payment_id AND payment_plan='consulation fee' AND paid=0";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }
    ...
    public function getStudentConsulationFeeShouldPay_old($id,$payment_id)
    {
        $qry = "SELECT SUM(amount) as consulation_fee_should_pay FROM `archive_payments` where student_id=$id and payment_id = $payment_id AND dead_line<= '".date('Y-m-d')."' AND payment_plan='consulation fee'";
        $query = $this->db->query($qry)->result_array();
        return $query;
    }

    public function getStudentTotalFine_old($student_id,$payment_id)
    {
        $this->db->select('*');
        $this->db->from('archive_payments');
        $this->db->where('student_id', $student_id);
    ...
    public function getStudentRemovedFine_old($student_id,$payment_id)
    {
        $this->db->select('*');
        $this->db->from('archive_payments');
        $this->db->where('student_id', $student_id);
    ...
    public function getStudentFineShouldPay_old($student_id,$payment_id)
    {
        $this->db->select('*');
        $this->db->from('archive_payments');
        $this->db->where('student_id', $student_id);
    ...
    public function getStudentFinePaid_old($student_id,$payment_id)
    {
        $payments = $this->db->get_where('archive_payments',array('student_id'=>$student_id,'paid'=>1,"payment_id",$payment_id))->result_array();
        $fine=0;
        foreach($payments as $payment)
        {
            $date_now = date("Y-m-d"); // this format is string comparable
    ...
    public function getStudentTotalCalls_old($student_id,$payment_id)
    {
        $payments = $this->db->get_where('archive_payments',array('student_id'=>$student_id,"payment_id",$payment_id))->result_array();
        $payment_ids=array();
        foreach($payments as $payment)
        {
            array_push($payment_ids,$payment['id']);
